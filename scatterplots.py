...
# Extract results
f_bar, X_bar_opt, log_c_opt, log_b_opt, log_sigma_sq_opt, X_bar_init = run_likelihood_optimization(N, D, M, X, y)

X_pred_full, y_mean_full, y_std_full, samples_full = GP_predict(X, y, X.min()-0.5, X.max()+0.5, plot=False)
X_pred_sparse, y_mean_sparse, y_std_sparse, samples_sparse = GP_predict(X_bar_opt, f_bar, X.min()-0.5, X.max()+0.5, plot=False)
...
plt.figure(2)
plt.plot(X_pred_sparse, y_mean_sparse, 'b-', label='Mean prediction')
plt.fill_between(
    X_pred_sparse.ravel(),
    y_mean_sparse.ravel() - 2 * y_std_sparse,
    y_mean_sparse.ravel() + 2 * y_std_sparse,
    color='blue',
    alpha=0.2,
    label='95% confidence interval'
